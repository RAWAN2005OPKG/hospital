            ...
            <div style="display: flex; flex-direction: column; gap: 2rem;">
                <div style="background: white; border-radius: 2rem; box-shadow: 0 20px 25px rgba(0,0,0,0.05); padding: 2.5rem; border: 1px solid #e5e7eb; text-align: center;">
                    ...
                    <span style="display: inline-block; padding: 0.4rem 1rem; background: #dbeafe; color: #1e40af; font-weight: bold; border-radius: 2rem; font-size: 0.85rem;">
                        <i class="fas fa-user-tag" style="margin-left: 0.25rem;"></i> {{ $doctor->user->isPatient() ? \'مريض\' : \'طبيب\' }}
                    </span>
                    ...
                    <div style="margin-top: 2rem; text-align: right; display: flex; flex-direction: column; gap: 1rem;">
                        ...
                        <div style="display: flex; align-items: center; gap: 0.75rem; color: #4b5563;">
                            ...
                            <span style="font-size: 0.9rem;">{{ $doctor->user->address ?? \'غير مسجل\' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div style="background: white; border-radius: 2rem; box-shadow: 0 20px 25px rgba(0,0,0,0.05); padding: 2rem; border: 1px solid #e5e7eb;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div style="background: #f0f9ff; padding: 1rem; border-radius: 1rem; text-align: center;">
                            <p style="font-size: 1.5rem; font-weight: 800; color: #3b82f6; margin: 0;">{{ $doctor->user->patient->appointments_count ?? 0 }}</p>
                        ...
                        <div style="background: #ecfdf5; padding: 1rem; border-radius: 1rem; text-align: center;">
                            <p style="font-size: 1.5rem; font-weight: 800; color: #10b981; margin: 0;">{{ $doctor->user->patient->prescriptions_count ?? 0 }}</p>
            ...
            <div style="display: flex; flex-direction: column; gap: 2rem;">
                ...
                <div style="background: white; border-radius: 2rem; box-shadow: 0 20px 25px rgba(0,0,0,0.05); padding: 2.5rem; border: 1px solid #e5e7eb;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                        ...
                        <div style="background: #fff5f5; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #fee2e2;">
                            ...
                            <p style="font-size: 1.5rem; font-weight: 900; color: #dc2626; margin: 0;">{{ $doctor->user->patient->blood_type ?? \'غير محدد\' }}</p>
                        </div>
                        @endif
                        @if($doctor->user->isPatient())
                        <div style="background: #f0fdf4; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #dcfce7;">
                            ...
                            <p style="font-size: 1.1rem; font-weight: 800; color: #15803d; margin: 0;">{{ $doctor->user->patient->birth_date ? \\Carbon\\Carbon::parse($doctor->user->patient->birth_date)->format(\'Y/m/d\') : \'غير مسجل\' }}</p>
                        </div>
                        <div style="background: #fefce8; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #fef08a;">
                            ...
                            <p style="font-size: 1.1rem; font-weight: 800; color: #a16207; margin: 0;">{{ $doctor->user->patient->gender === \'male\' ? \'ذكر\' : ($doctor->user->patient->gender === \'female\' ? \'أنثى\' : \'غير محدد\') }}</p>
                        </div>
                        @else
                        <div style="background: #f0fdf4; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #dcfce7;">
                        ...
                        <div style="background: #fefce8; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #fef08a;">
                            ...
                            <p style="font-size: 1.1rem; font-weight: 800; color: #a16207; margin: 0;">{{ $doctor->specialty->name ?? \'عام\' }}</p>
                        </div>
                        @endif
                        @if($doctor->user->isPatient())
                        <div style="background: #f5f3ff; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #ddd6fe;">
                            ...
                            <p style="font-size: 1rem; font-weight: 800; color: #7c3aed; margin: 0;">{{ $doctor->user->patient->emergency_contact ?? \'غير مسجل\' }}</p>
                        </div>
                        @else
                        <div style="background: #f5f3ff; padding: 1.25rem; border-radius: 1.5rem; border: 1px solid #ddd6fe;">
                            <p style="font-size: 0.85rem; color: #5b21b6; font-weight: bold; margin: 0 0 0.25rem 0;">سنوات الخبرة</p>
                            <p style="font-size: 1rem; font-weight: 800; color: #7c3aed; margin: 0;">{{ $doctor->experience_years ?? 0 }} سنوات</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div style="background: white; border-radius: 2rem; box-shadow: 0 20px 25px rgba(0,0,0,0.05); padding: 2.5rem; border: 1px solid #e5e7eb;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        @if($doctor->user->patient && $doctor->user->patient->medicalRecords)
                            @forelse($doctor->user->patient->medicalRecords()->latest()->take(3)->get() as $record)
